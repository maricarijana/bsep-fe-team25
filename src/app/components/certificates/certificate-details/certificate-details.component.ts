...
export class CertificateDetailsComponent implements OnInit {
  ...
  loadCertificate(): void {
    ...
    this.certificateService.getMyCertificates().subscribe({
      next: (certificates) => {
        this.certificate = certificates.find(c => c.serialNumber === this.serialNumber) || null;
        if (!this.certificate) {
          this.snackBar.open('Certificate not found', 'Close', { duration: 3000 });
  ...
  downloadPEM(): void {
    ...
    const blob = new Blob([this.certificate.pemCertificate], { type: 'application/x-pem-file' });
  ...
  copyPEM(): void {
    ...
    navigator.clipboard.writeText(this.certificate.pemCertificate).then(() => {
      this.snackBar.open('PEM copied to clipboard', 'Close', { duration: 2000 });
    });
  }
